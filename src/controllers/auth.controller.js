...
export const loginUser = async (req, res) => {
    ...
    try{
        ...
        return res.json({message:'Login successful'}, {accessToken: varAccess})
...
export const updateUser = async(req, res) => {
    ...
    try{
        userProfile = await User.findOne({name})

        const newHash = await bcrypt.hash(newpassword, 10);

        const newUser = await User.updateOne({name}, {password: newHash});
